...
* **Omgeving**: set met een aanbiedingscatalogus en haken (aanbiedingsruimten). Maak één omgeving door dimensie als doel in te stellen. Er zijn twee soorten omgevingen:
...
* **Ruimte voor aanbod**: map waarin de locatie wordt gedefinieerd waar de aanbieding wordt weergegeven. Wanneer u een spatie definieert, kunt u:
...
* **Arbitrage**: aanbiedingen selecteren die worden weergegeven in een omgeving (in aanmerking komende aanbiedingen). Het arbitragebeginsel rangschikt aanbiedingen op de rangorde volgens de criteria die in de categorieën, aanbiedingen en contextaanbiedingen worden bepaald.
...
* **Uitgaande interactie**: bel de motor van de Aanbieding van een contactlijst (gebruikt voor het leveren van e-mail, direct post, enz.). De zelfde regels en de processen worden toegepast op elk contact. Dit type interactie wordt meestal verwerkt in batchmodus.
...
* **Eenvoudige modus**: er wordt één contactpersoon tegelijk verwerkt. Deze wijze wordt gewoonlijk toegepast voor binnenkomende interactie en transactionele berichten.
* **Identificatiemodus**: verwijst naar de status van een contactpersoon:
   ...
   * **[!UICONTROL implicit]** : contact wordt geïdentificeerd door een cookie (permanent of sessie). Het kan als anoniem of geïdentificeerd contact worden verwerkt.
...
* **Renderfunctie**: functie die in de aanbiedingsruimte wordt gedefinieerd om de aanbiedingsweergave te construeren op basis van de kenmerken die in de aanbieding zijn gedefinieerd. Er zijn drie verschillende renderingfuncties: HTML, XML en tekst.
