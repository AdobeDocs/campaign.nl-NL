...
* **de Eenvoudige Dienst van de Opslag van Amazon (S3)** - de **schakelaar van AWS S3** kan worden gebruikt om gegevens in te voeren of uit te voeren naar Adobe Campaign gebruikend de activiteit van het a **[!UICONTROL Transfer file]** werkschema. Wanneer u dit nieuwe externe account instelt, moet u de volgende data opgeven:
   ...
   * **[!UICONTROL AWS access key ID]**: Leer hoe te om uw de toegangs belangrijkste identiteitskaart van AWS in [ documentatie van Amazon ](https://docs.aws.amazon.com/general/latest/gr/aws-sec-cred-types.html#access-keys-and-secret-access-keys) te vinden {target="_blank"}.

   * **[!UICONTROL Secret access key to AWS]**: Leer hoe te om uw geheime toegangstoets aan AWS in [ documentatie van Amazon ](https://aws.amazon.com/fr/blogs/security/wheres-my-secret-access-key/) te vinden {target="_blank"}.
