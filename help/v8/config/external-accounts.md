...
#### Externe account voor Snowflake {#snowflake-external-accounts}

De Snowflake FDA-verbinding gebruikt het Snowflake ODBC-stuurprogramma. Vanaf Campagne v8.9.1 ondersteunen externe accounts van Snowflake OAuth2-verificatie, waardoor beveiligde verificatie voor gefedereerde gegevenstoegang mogelijk is.

Leer meer over OAuth in Snowflake in de [&#x200B; documentatie van Snowflake &#x200B;](https://docs.snowflake.com/en/user-guide/oauth-intro){target="_blank"}.

Eerst moet u de volgende stappen uitvoeren op Snowflake:

1. Voordat u uw externe Snowflake-account kunt configureren met OAuth 2.0, moet u eerst een OAuth Security Integration in Snowflake maken. De **ACCOUNTADMIN** rol wordt vereist om de veiligheidsintegratie tot stand te brengen.

   Leer meer over het creëren van de Integratie van de Veiligheid OAuth in de [&#x200B; documentatie van Snowflake &#x200B;](https://docs.snowflake.com/en/sql-reference/sql/create-security-integration-oauth-snowflake){target="_blank"}.

1. U kunt dan de identiteitskaart van de Cliënt en Geheim gebruiken vragen van de Cliënt:

   ```
   select system$show_oauth_client_secrets('OAUTH_INTEGRATION_ABC'); // use uppercase letters
   ```

Ga als volgt te werk om OAuth2-verificatie in Campagne te configureren:

1. Maak of bewerk in Adobe Campaign een externe Snowflake-account en schakel de optie **[!UICONTROL Use OAuth 2.0]** in.

1. Stel de server, database en schema in en open het tabblad **[!UICONTROL OAuth]** .

1. Stel de parameters **[!UICONTROL Client ID]** , **[!UICONTROL Client Secret]** en **[!UICONTROL Redirect URL]** voor de integratie van de beveiliging in. Deze parameters worden verkregen uit uw Snowflake OAuth Security Integration. Verwijs naar de [&#x200B; documentatie van Snowflake &#x200B;](https://docs.snowflake.com/en/user-guide/oauth-custom){target="_blank"}.

1. Klik op **[!UICONTROL Proceed to Sign in]** om handmatig aan te melden. Er wordt een nieuw browservenster geopend waarin u wordt gevraagd uw gebruikersgegevens voor Snowflake in te voeren.

1. Nadat het verificatieproces is voltooid, wordt het account geverifieerd voor het aantal dagen dat is gedefinieerd in de Snowflake OAuth Security Integration (met de parameter `OAUTH_REFRESH_TOKEN_VALIDITY` ). Het token voor vernieuwen wordt opgeslagen in de externe account.

>[!CAUTION]
>
>De omleidings-URL moet altijd gericht zijn op `oauth.jsp` op de computer met de campagnetoepassingsserver via HTTPS (poort 443). Serverdomeinen met onderstrepingstekens worden ook niet ondersteund bij gebruik van OAuth. Gebruik serverdomeinen zonder onderstrepingstekens waar het de bedoeling is om OAuth te gebruiken.
